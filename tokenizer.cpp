...
void 
push_back(token_vec& arg_vec, std::string& arg_str)
{
    arg_vec.push_back(arg_str);
    arg_str = "";
}

void push_back_brace
(token_vec& token_vec, char ch, std::string& tmp_str, int& counter)
{
	if("" != tmp_str) {
...
(token_vec& token_vec, const std::string& arg_str, std::string& tmp_str, int& i)
{
	tmp_str += arg_str[i++];
	int counter = 0;
	while(true){
...
token_vec tokenize_script(const std::string& arg_str)
{
	...
	//std::cout << counter*1000000 << std::endl; //  TO BE DELETED
	/*
	   if(0 != counter) {
		   throw
	   }
